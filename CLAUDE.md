...
3. Latest release asset on the **private** repo `Jani7/ria-ma-data`, fetched with the GitHub PAT from Streamlit secret `ADV_DATA_TOKEN`.
4. If none of the above resolve, the lookup UI shows "SEC data file not found" and the rest of the app keeps working.

**One-time production setup:**
1. Create a private GitHub repo named `ria-ma-data` under the `Jani7` account.
2. Run `python scripts/build_adv_data.py` locally, then upload the resulting `data/adv_current.parquet` as an asset on a release in that private repo (any tag, e.g. `v2026-05`).
...
4. In Streamlit Cloud → app → Settings → Secrets, add:
   ...
   # ADV_DATA_ASSET = "adv_current.parquet"
   ```
5. Reboot the app. On first lookup it fetches the asset into `/tmp` and caches it for the container.

**Quarterly refresh:** re-run `scripts/build_adv_data.py`, upload the new Parquet as a new release on `Jani7/ria-ma-data`. The app picks up the latest release automatically on next cold start.
